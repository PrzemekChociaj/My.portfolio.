  ...
  return (
    <section>
      <div name='testimonials' className="h-screen bg-[#252A34] flex items-center flex-col text-center px-5 pt-14">
        <h2 className="text-white font-bold mt-10 text-center text-3xl border-b-2 inline border-[#FF2E63]">
          {t("Testimonial.1")}
        </h2>
        <br />
        <Swiper
        ...
        >
          {translatedArray.map((item, index) => (
            <SwiperSlide key={index}>
              <div
                className={`duration-400 bg-[#393E48] mx-8 p-8 h-full rounded-3xl flex items-center gap-6 md:flex-row flex-col text-white italic ${activeIndex !== index && "scale-76 blur-sm"}`}
              >
                <img src={item.authorIMG} alt="..." className="h-24 rounded-full" />
                <div>
